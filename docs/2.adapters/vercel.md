...
Use `handleWebUpgrade` from runtimes that receive a Web `Request` and return a `Response`.
...
export default {
  async fetch(request) {
    const response = await ws.handleWebUpgrade(request);
    if (response) {
      return response;
    }
...
Use `handleNodeUpgrade` from Vercel Node.js handlers that receive an `IncomingMessage` and `ServerResponse`.
...
export default async function handler(req, res) {
  if (await ws.handleNodeUpgrade(req, res)) {
    return;
  }
...
`handleWebUpgrade` returns `undefined` and `handleNodeUpgrade` returns `false` when the request is not a WebSocket upgrade or when Vercel's upgrade context is unavailable. When a WebSocket upgrade is handled, the returned fetch response or Node response status is `204`; the WebSocket handshake itself is completed on the raw upgrade socket.
