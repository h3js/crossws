...
| [`broadcastChannel`](#broadcastchannel-channel) | `BroadcastChannel` | Single host (worker threads / processes) | Local fan-out, tests, single-host multi-worker deployments. |
...
Unlike Redis `SUBSCRIBE`, Postgres `LISTEN` doesn't block the connection, so the same client both listens and notifies — no duplicate connection needed. Note Postgres caps a `NOTIFY` payload at 8000 bytes, so keep relayed messages small.
